...
namespace K4os.Compression.LZ4.Internal
{
	...
	public unsafe class Mem
	{
		...
		// Poor mans version of MemCpy implemented since we can't access the one in System.Runtime.CompilerServices.Unsafe
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static void PoorMansMemCpy(void* target, void* source, uint length) {
			for (uint i = 0; i < length; i++)
				((byte*)target)[i] = ((byte*)source)[i];
		}
		
		// Poor mans version of MemSet implemented since we can't access the one in System.Runtime.CompilerServices.Unsafe
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static void PoorMansMemSet(void* target, byte value, uint length) {
			for (uint i = 0; i < length; i++)
				((byte*)target)[i] = value;
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static void CpBlk(void* target, void* source, uint length) => PoorMansMemCpy(target, source, length);
		...
		/// <param name="length">Length in bytes.</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static void ZBlk(void* target, byte value, uint length) => PoorMansMemSet(target, value, length);
		...
		public static void* CloneAnyArray<T>(T[] array) {
			...
			var ret = Marshal.AllocHGlobal(size * array.Length);
			for(var i = 0; i < array.Length; i++)
				Marshal.StructureToPtr(array[i], ret + i * size, false);
