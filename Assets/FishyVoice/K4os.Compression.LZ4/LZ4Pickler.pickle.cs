...
namespace K4os.Compression.LZ4
{
	...
	public static partial class LZ4Pickler
	{
		...
		private static unsafe void PoorMansMemCpy(ref byte target_, ref byte source_, uint length) {
			fixed (byte* target = &target_) 
				fixed (byte* source = &source_) 
		...
		private static unsafe void PokeN(Span<byte> target, int value, int size)
		{
			...
			PoorMansMemCpy(ref target[0], ref *(byte*)&value, (uint)size);
