...
public class VoiceProcessor : MonoBehaviour {
	...
	[SerializeField]
	[Tooltip("The minimum volume to detect voice input for")]
	[Range(0.0f, 1.0f)]
		...
		while (IsRecording) {
			...
			if (_autoDetect == false) {
				...
				if (maxVolume >= _minimumSpeakingSampleValue) {
					...
					if (_audioDetected && Time.time - _timeAtSilenceBegan > _silenceTimer) _audioDetected = false;
				}
			}
