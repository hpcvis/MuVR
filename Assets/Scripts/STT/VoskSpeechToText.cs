...
public class VoskSpeechToText : MonoBehaviour {
	...
	///     A list of keywords/phrases. Keywords need to exist in the models dictionary, so some words
	///     like "webview" are better detected as two more common words "web view".
	/// </param>
	/// <param name="modelPath">
	///     The path to the model folder relative to StreamingAssets. If the path has a .zip ending, it
	///     will be decompressed into the application data persistent folder.
	/// </param>
	...
	public void StartVoskStt(List<string> keyPhrases = null, string modelPath = default, bool startMicrophone = false,
		...
		if (keyPhrases is not null) KeyPhrases = keyPhrases;
	...
	private void UpdateGrammar() {
		...
		foreach (var keyphrase in KeyPhrases) keywords.Add(new JSONString(keyphrase.ToLower()));
	...
	private IEnumerator Decompress() {
		if (!Path.HasExtension(ModelPath)
		    || Directory.Exists(Path.Combine(Application.persistentDataPath, ModelPath)) 
		...
		var dataPath = Path.Combine(Application.streamingAssetsPath, ModelPath);

		Stream dataStream;
		// Read data from the streaming assets path. You cannot access the streaming assets directly on Android.
		if (dataPath.Contains("://")) {
			var www = UnityWebRequest.Get(dataPath);
			www.SendWebRequest();
		...
		while (_isDecompressing == false) yield return null;
	...
	private void ZipFileOnExtractProgress(object sender, ExtractProgressEventArgs e) {
		if (e.EventType != ZipProgressEventType.Extracting_AfterExtractAll) return;
	...
	private void Update() {
		if (_threadedResultQueue.TryDequeue(out var voiceResult)) OnTranscriptionResult?.Invoke(voiceResult);
	}
	...
	private async Task ThreadedWork() {
		...
		if (!_recognizerReady) {
			...
			_recognizer = string.IsNullOrEmpty(_grammar) ? new VoskRecognizer(_model, 16000.0f) : new VoskRecognizer(_model, 16000.0f, _grammar);
		...
		while (_running)
			if (_threadedBufferQueue.TryDequeue(out var voiceResult)) {
				if (!_recognizer.AcceptWaveform(voiceResult, voiceResult.Length)) continue;
