...
public class RecognitionResult {
	...
	public RecognitionResult(string json) {
		var resultJson = JSONNode.Parse(json).AsObject;
		...
		if (resultJson.HasKey(AlternativesKey)) {
			...
			for (var i = 0; i < Phrases.Length; i++) Phrases[i] = new RecognizedPhrase(alternatives[i].AsObject);
		} else if (resultJson.HasKey(ResultKey))
			Phrases = new[] { new RecognizedPhrase(resultJson.AsObject) };
		else if (resultJson.HasKey(PartialKey)) {
			Partial = true;
			Phrases = new[] { new RecognizedPhrase { Text = resultJson[PartialKey] } };
		} else
...
public class RecognizedPhrase {
	...
	public RecognizedPhrase(JSONObject json) {
		if (json.HasKey(ConfidenceKey)) Confidence = json[ConfidenceKey].AsFloat;
