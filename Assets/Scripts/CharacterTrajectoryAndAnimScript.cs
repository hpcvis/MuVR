...
public class CharacterTrajectoryAndAnimScript : MonoBehaviour {
	...
	public void UpdateTargetDirectionAndVelocity(Vector3 newTargetDirection, float axisX, float axisY, float rightTrigger) {
		...
		var newTargetVelocity = movementSpeed * (newTargetRotation * new Vector3(axisX, 0.0f, axisY));
	...
	public void UpdateGait(float rightTrigger) {
		if (targetVelocity.magnitude < 0.1f) { // Standing still
			var standAmount = 1.0f - Mathf.Clamp01(targetVelocity.magnitude / 0.1f);
	...
	public void PredictFutureTrajectory() {
		...
		for (var i = trajectoryLength / 2 + 1; i < trajectoryLength; i++) {
			var biasPosition = Mathf.Lerp(0.5f, 1.0f, strafeAmount); // On both variables will come character response check (569)
			var biasDirection = Mathf.Lerp(2.0f, 0.5f, strafeAmount);

			var scalePosition = 1.0f - Mathf.Pow(1.0f - (float)(i - trajectoryLength / 2) / (trajectoryLength / 2), biasPosition);
			var scaleDirection = 1.0f - Mathf.Pow(1.0f - (float)(i - trajectoryLength / 2) / (trajectoryLength / 2), biasDirection);
			...
			for (var j = 0; j < terrainWalls.Length; j++) {
				...
				if (segmentDistance < wallWidth + wallVal) {
					var point0 = (wallWidth + 0.0f) * (trajectoryPoint - segmentPoint).normalized + segmentPoint;
	...
	public void UpdateHeights() {
		...
		for (var i = 0; i < trajectoryLength; i += 10) points[trajectoryLength / 2].height += points[i].position.y / (trajectoryLength / 10);
	}
	...
	*/
	...
	public void UpdateNetworkInput(ref Matrix X) {
		...
		o += jointsNumber * 3 * 2;
		for (var i = 0; i < trajectoryLength; i += 10) {
			var positionRight = points[i].position + points[i].rotation * new Vector3(sidePointsOffset, 0.0f, 0.0f);
			var positionLeft = points[i].position + points[i].rotation * new Vector3(-sidePointsOffset, 0.0f, 0.0f);
	...
	public void PostVisualisationCalculation(Matrix Y) {
		...
		var trajectoryUpdate = points[trajectoryLength / 2].rotation * new Vector3(Y[0], 0.0f, Y[1]);
		...
		points[trajectoryLength / 2].direction = Quaternion.AngleAxis(standAmount * -Y[2, 0] * Mathf.Rad2Deg, Vector3.up)
		                                         * points[trajectoryLength / 2].direction; // new Vector3(0.0f, 1.0f, 0.0f)
		...
		points[trajectoryLength / 2].rotation = Quaternion.AngleAxis(
												...
												* Mathf.Rad2Deg, Vector3.up); //new Vector3(0.0f, 1.0f, 0.0f));
		...
		for (var i = trajectoryLength / 2 + 1; i < trajectoryLength; i++) {
			var m = (i - trajectoryLength / 2 / 10.0f) % 1.0f;
	...
	public void DisplayTrajectory() {
		...
		for (var i = 0; i < trajectoryLength; i += 10) {
			// left
			var posLeft = Vector3.up + points[i].rotation * new Vector3(-sidePointsOffset * scaleFactor, 0.0f, 0.0f);
			points[i].framePoint.transform.GetChild(0).localPosition = posLeft;

			// right
			var posRight = Vector3.up + points[i].rotation * new Vector3(sidePointsOffset * scaleFactor, 0.0f, 0.0f);
